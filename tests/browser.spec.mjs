...
before(async()=>{
  ...
  server=createServer(async(req,res)=>{
    ...
    if(!['index.html','app.js','news-model.js','style.css','assets/stringer-logo.png','data/news.json'].includes(file)){res.writeHead(404);res.end();return;}
...
async function pageAt(path='/',width=1280){const page=await browser.newPage({viewport:{width,height:1000}});page.on('pageerror',error=>errors.push(error.message));await page.goto(base+path);await page.waitForSelector('#stringer-news article');return page;}
test('desktop, 768px, and mobile layouts render two populated columns without overflow',async()=>{
  for(const width of [1280,768,375]){
    ...
    assert.equal(await page.locator('.brand img').evaluate(el=>el.complete && el.naturalWidth>0),true);
    await page.screenshot({path:`.test-output/news-${width}.png`,fullPage:false});await page.close();
  }
...
test('refresh of the same snapshot preserves timestamps and reports no newer collection',async()=>{
  const page=await pageAt();const status=await page.locator('#world-status').innerText();await page.locator('#refresh').click();await page.waitForFunction(()=>document.querySelector('#refresh-status').textContent.startsWith('No newer'));
  assert.equal(await page.locator('#world-status').innerText(),status);await page.close();
});
test('failed browser refresh retains displayed stories; first-load failure is visible',async()=>{
  const page=await pageAt();await page.route('**/data/news.json',route=>route.fulfill({status:503,body:'Unavailable'}));await page.locator('#refresh').click();await page.waitForFunction(()=>document.querySelector('#refresh-status').textContent.includes('Could not check'));
  assert.equal(await page.locator('#stringer-news article').count(),24);assert.equal(await page.locator('#refresh').isEnabled(),true);
...
test('retained-source failures, valid empty newswire, and stopped-schedule staleness are displayed',async()=>{
  ...
  await page.locator('#refresh').click();await page.waitForSelector('#world-news .empty');assert.match(await page.locator('#world-status').innerText(),/No stories returned/);
...
test('malicious snapshot is rejected and remote markup is rendered as text',async()=>{
  const page=await pageAt();const bad=structuredClone(snapshot);bad.sources.stringer.stories[0].url='javascript:alert(1)';await page.route('**/data/news.json',route=>route.fulfill({json:bad}));await page.locator('#refresh').click();await page.waitForFunction(()=>document.querySelector('#refresh-status').textContent.includes('Could not check'));assert.equal(await page.locator('#stringer-news article').count(),24);
  bad.sources.stringer.stories[0].url='https://example.org/story';bad.sources.stringer.stories[0].title='<img src=x onerror=alert(1)>';await page.locator('#refresh').click();await page.waitForFunction(()=>document.querySelector('#stringer-news h3').textContent.startsWith('<img'));
...
test('visible polling, hidden-page pause, visibility catch-up, and overlap prevention',async()=>{
  ...
  await page.clock.fastForward(300_000);await page.waitForFunction(()=>!document.querySelector('#refresh').disabled);assert.equal(requests,2);
  ...
  await page.evaluate(()=>{Object.defineProperty(document,'hidden',{configurable:true,get:()=>false});document.dispatchEvent(new Event('visibilitychange'))});await page.waitForFunction(()=>!document.querySelector('#refresh').disabled);assert.equal(requests,3);
  release=null;await page.locator('#refresh').click();await page.waitForTimeout(25);assert.equal(requests,4);await page.evaluate(()=>{document.querySelector('#refresh').dispatchEvent(new Event('click'));document.dispatchEvent(new Event('visibilitychange'))});await page.clock.fastForward(1000);assert.equal(requests,4);assert.equal(await page.locator('#refresh').isDisabled(),true);release();await page.waitForFunction(()=>!document.querySelector('#refresh').disabled);await page.close();
});
test('keyboard navigation and requested removals',async()=>{
  ...
  assert.equal(await page.locator('#reporter-filter,.topics,#reporter-directory').count(),0);assert.doesNotMatch(await page.locator('body').innerText(),/Nostr|Mastodon|About Nostr|All sources/);
